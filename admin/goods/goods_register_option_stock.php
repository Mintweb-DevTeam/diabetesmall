    ...
    $(document).ready(function () {
        ...
        $("#frmGoods").validate({
            submitHandler: function (form) {
				...
				oEditors.getById["subscription_editor"].exec("UPDATE_CONTENTS_FIELD", []);	// 에디터의 내용이 textarea에 적용됩니다.
        ...
        <table class="table table-cols">
            ...
            <tr>
				...
				<td >
					<div class="radio">
                        <label class="radio-inline" >
                            <input type="radio" name="coViewOrder" value="n" checked /> 보이지 않음
                        </label>
                        <label class="radio-inline" title="">
                            <input type="radio" name="coViewOrder" value="y" <?=($data['coViewOrder'] == 'y') ? 'checked' : '' ?> /> 보여줌
                        </label>
                    </div>
				</td>
            </tr>
            ...
            <tr>
				<th>이벤트 상품 여부</th>
				<td>
					<label><input name="isEvent" type="radio" value="n" checked>일반 상품</label>
					<label style="margin-left: 1.5em"><input name="isEvent" type="radio" value="y" <?=$data['isEvent']=='y'?'checked':''?>>이벤트 상품</label>
				</td>
                <th>상품 노출시간</th>
                <td>
                    <div class="form-inline">
...
" target="_blank" >전기용품 및 생활용품 안전관리법 가이드라인</a>의 내용을 확인해 주세요.
    ...
    <div class="table-title gd-help-manual" id="subscriptionDetail">
        정기배송상품 설명
        <span class="depth-toggle"><button type="button" class="btn btn-sm btn-link bold depth-toggle-button" depth-name="goodsSpec"><span>닫힘</span></button></span>
    </div>
    <div id="depth-toggle-layer-subscriptionDetail">
                <div id="textareaSpecShop">
                    <textarea name="subscriptionDetail" rows="3" style="width:100%; height:400px;" id="subscription_editor" class="form-control"><?=$data['subscriptionDetail']; ?></textarea>
                </div>
	</div>
	...
	<div>
		<label title="" class="label-width" style="position: relative;">
			<input type="text" name="subscription_shortDescription" value="<?=$data['subscription_shortDescription']?>" class="form-control input-width js-maxlength" maxlength="250">
		</label>
	</div>
	<div class="table-title">
		정기배송상품 상단짧은설명
	...
	<div>
		...
		<label title="" class="label-width" style="position: relative;">
			<input type="text" name="subscription_top_detail" value="<?=$data['subscription_top_detail']?>" class="form-control input-width js-maxlength" maxlength="250">
		</label>
	</div>
	<div class="table-title">
		정기배송상품 하단짧은설명
	...
	<div>
		<label title="" class="label-width" style="position: relative;">
			<input type="text" name="subscription_bottom_detail" value="<?=$data['subscription_bottom_detail']?>" class="form-control input-width js-maxlength" maxlength="250">
		</label>
	</div>
...
<script type="text/javascript">
    ...
    nhn.husky.EZCreator.createInIFrame({
        oAppRef: oEditors,
        elPlaceHolder: "subscription_editor",
        sSkinURI: "<?=PATH_ADMIN_GD_SHARE?>script/smart/SmartEditor2Skin.html",
        htParams: {
            bUseToolbar: true,				// 툴바 사용 여부 (true:사용/ false:사용하지 않음)
            bUseVerticalResizer: true,		// 입력창 크기 조절바 사용 여부 (true:사용/ false:사용하지 않음)
        ...
        fOnAppLoad: function () {
            ...
            //oEditors.getById["editor"].exec("PASTE_HTML", ["로딩이 완료된 후에 본문에 삽입되는 text입니다."]);
            $("#textareaDescriptionMobile").hide();
            toggleSelectionDisplay('subscriptionDetail');
        },
        fCreator: "createSEditor2"
    });
