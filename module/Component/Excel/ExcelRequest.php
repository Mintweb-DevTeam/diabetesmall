...
//2021.09.16민트웹 정기결제회원추가 해당함수 상속안됨
                ...
                foreach ($excelField as $excelKey => $excelValue) {
                    ...
                    switch ($excelValue) {
						...
						case 'subscription':
								...
								//2021.09.16민트웹 정기결제회원추가
								$now_date = date("Y-m-d");
								$sql="SELECT count(a.memNo) as cnt FROM wm_subscription_apply a INNER JOIN `wm_subscription_schedule_list` b ON a.uid=b.uid WHERE b.isStop='0' and a.memNo='{$val['memNo']}' and from_unixtime(b.schedule_stamp,'%Y-%m-%d')>='$now_date'";
            ...
            foreach($data as $key => $val) {
                ...
                foreach ($excelField as $excelKey => $excelValue) {
                    ...
                    switch ($excelValue) {
						...
						case "period":
							$sql="select a.idx,b.period from wm_subscription_schedule_list a INNER JOIN wm_subscription_apply b ON a.uid=b.uid where a.orderNo='{$val['orderNo']}'";
							$row = $db->fetch($sql);
							if(!empty($row['period'])){
						...
						case "schedule":
							...
							if(!empty($krow['uid'])){
								...
								$sql="select count(idx) as cnt from wm_subscription_schedule_list where uid='{$krow['uid']}' and orderNo<>''";
								$rows = $db->fetch($sql);
								$order_cnt =$rows['cnt'];

								 $tmpData[] = "<td style=mso-number-format:'\@'>".$order_cnt."/".$tcount."</td>";
							}
						...
						case 'hospitalNm' :
							$sql="select `hospitalNm` from `es_orderInfo` where `orderNo`='{$val['orderNo']}'";
                            $tmpData[]="<td>". $db->query_fetch($sql)[0]['hospitalNm'] . "</td>";
						break;
						case 'hospitalAdd' :
							$sql="select `hospitalAdd` from `es_orderInfo` where `orderNo`='{$val['orderNo']}'";
                            $tmpData[]="<td>". $db->query_fetch($sql)[0]['hospitalAdd'] . "</td>";
						break;							
							
						case "schedule_tcount":
						...
						case "autoExtend":
							$sqls="select a.idx,b.period,b.deliveryEa,b.autoExtend from wm_subscription_schedule_list a INNER JOIN wm_subscription_apply b ON a.uid=b.uid where a.orderNo='{$val['orderNo']}'";
							$row = $db->fetch($sqls);
						...
						case "autoExtendCount":
							...
							if(!empty($krow['uid'])){
								...
								$logRow=$db->fetch($logSQL);
								$tmpData[] = "<td>".$logRow['cnt']."회</td>";
							}else{
								$tmpData[] = "<td>&nbsp;</td>";
							}
						...
						case "gift":
							...
							//사은품 존재 여부체크시작
							//2024.01.31웹앤모바일 1000000060상품조건 추가함
							$strSQL="select count(a.idx) as cnt from wm_subscription_schedule_list a INNER JOIN wm_subscription_apply_items b ON b.uid=a.uid where a.orderNo='{$val['orderNo']}' and b.goodsNo='1000000060'";
							$strRow=$db->query_fetch($strSQL);
							
							if($strRow[0]['cnt']>0){
								$tmpData[] = "<td>12회 센서증정</td>";
							}else{
								$giftSQL="select count(orderNo) as cnt from ".DB_ORDER_GIFT." where orderNo=?";
								$giftROW=$db->query_fetch($giftSQL,['i',$val['orderNo']],false);
								
								if($giftROW['cnt']>0)
									$tmpData[] = "<td>추가증정대상</td>";
								else
									$tmpData[] = "<td>&nbsp;</td>";
								//사은품 존재 여부체크종료
							}
