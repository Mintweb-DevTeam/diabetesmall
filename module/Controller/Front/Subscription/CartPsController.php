         ...
         switch ($in['mode']) {
             case "cartIn" : 
				 ...
				 if($in['goods_view']=='1'){
					...
					if(!empty($memNo)){
						$sql="delete from wm_subscription_cart where memNo=?";
						$db->bind_query($sql,['i',$memNo]);
					}else{
						$siteKey = \Session::get("siteKey");
						$sql="delete from wm_subscription_cart where siteKey=?";
						$db->bind_query($sql,['s',$siteKey]);
					}

				 }
				 $request = \App::getInstance('request');
				 $cartIdx = $obj->saveInfoCart($in);

				 if ($request->isAjax()) {
					 if(empty($memNo)){
						 \Session::set('cartIdx', $cartIdx);
			
					 }
						//if(\Request::getRemoteAddress() =="182.216.219.157"){
							//2024.06.20웹앤모바일 추가시작-어도비 전송을 위해 json데이터로처리함
							 if (gd_isset($in['cartMode']) == 'd') {
								$subObj= new \Component\Subscription\Subscription();
								$subCfg = $subObj->getCfg();
								 
								$dataArr['period']=end($subCfg['period']);
								$dataArr['deliveryEa']=end($subCfg['deliveryEa']);
								
								$this->json($dataArr);
							 }
							 //2024.06.20웹앤모바일 추가종료
						//}else{
						//	echo "1";
						//}
                    } else {
						...
						?>
						<form name="csForm" method="post" action="../subscription/order.php">
						...
						<?php
                            //$returnUrl = '../subscription/order.php';
                        } 
