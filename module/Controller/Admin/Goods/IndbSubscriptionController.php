            ...
            case "update_config" : 
               ...
               /* $sql = "UPDATE wm_subscription_config 
                                SET 
									...
									am_type='{$in['am_type']}',
									am_deliveryDays='{$in['am_deliveryDays']}',
									af_type='{$in['af_type']}',
									af_deliveryDays='{$in['af_deliveryDays']}'";
								*/
							$sql = "UPDATE wm_subscription_config 
                                SET 
                                    pg= '{$in['pg']}',
              ...
              case "update_goods_config_list" : 
                ...
                foreach ($in['goodsNo'] as $goodsNo) {
					...
					$listViewFl =  $in['listViewFl'][$goodsNo]?'y':'n';

					$autoExtend = $in['autoExtend'][$goodsNo]?'y':'n';
					$orderBtn = $in['orderBtn'][$goodsNo]?'y':'n';
                    $sql = "UPDATE " . DB_GOODS . " SET isSubscription='{$isSubscription}',listViewFl='{$listViewFl}',autoExtend='{$autoExtend}',orderBtn='{$orderBtn}' WHERE goodsNo='{$goodsNo}'";
                    $db->query($sql);
                }
