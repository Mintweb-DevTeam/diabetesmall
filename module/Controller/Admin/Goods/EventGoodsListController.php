...
class EventGoodsListController extends \Controller\Admin\Controller 
{
    public function index()
    {
        $this->callMenu('goods', 'event_goods', 'event_goods_list');
        ...
        if ($get['start_goods'])
            $q[] = "(select count(wg.idx) as cnt from wm_event_goods wg where wg.goodsNo=a.goodsNo)>'0'";
        ...
        if ($tmp = $db->query_fetch($sql)) {
          foreach ($tmp as $t) {
			...
			$sql="select * from wm_event_goods where goodsNo='{$t['goodsNo']}'";
			...
			$oldSQL="select * from wm_start_goods where goodsNo='{$t['goodsNo']}'";
			$oldStartRow = $db->fetch($oldSQL);
			
			if(!empty($oldStartRow['idx'])){
				$goods['old_start_goods'] =1;
			}else{
				$goods['old_start_goods'] =0;
			}			
