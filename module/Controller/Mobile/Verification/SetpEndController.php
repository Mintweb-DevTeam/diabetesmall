...
class SetpEndController extends \Controller\Mobile\Controller
{
	public function index()
	{
		...
		//if(\Request::getRemoteAddress()=="182.216.219.157"){
			...
			//$msql="select privateConsignFl from ".DB_MEMBER." where memNm='{$row['memNm']}' and replace(cellPhone,'-','') = replace('{$row['cellPhone']}','-','') and memId='{$row['email']}'";	
		...
		if($result){
			echo '<script>alert("감사합니다. 처리되었습니다.");parent.location.replace("/verification/");</script>';
		}else{
